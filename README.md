...
🔗 **Acesse:** [rafa-britto-dp6.github.io/dp6-certificacoes](https://rafa-britto-dp6.github.io/dp6-certificacoes/)
...
- **`GET`** — carrega o store completo do banco
- **`PATCH`** — salva o store atualizado no banco
- **`OPTIONS`** — responde ao preflight CORS automaticamente
...
- [Supabase CLI](https://supabase.com/docs/guides/cli) instalado via Scoop (Windows):
...
```javascript
const EDGE_URL        = 'https://SEU_PROJECT_ID.supabase.co/functions/v1/dp6-store';
...
- **`consulting`** (azul claro) — Consulting / BA
- **`de`** (verde) — Data Engineering
- **`ds`** (roxo) — Data Science
- **`infra`** (laranja) — Infra / Help Desk
- **`liderancas`** (salmão) — Lideranças / Managers
- **`todos`** (cyan) — Todos os times
- **`outros`** (cinza) — Personalizado (campo livre)
...
- **`--bg-deep`** `#0a1520` — background principal
- **`--bg-card`** `#112236` — cards e painéis
- **`--cyan`** `#00c2d4` — destaque, CTAs e elementos primários
- **`--muted`** `#6a8a9f` — textos secundários
...
- **Supabase PostgreSQL** — banco de dados relacional com JSONB
- **Supabase Edge Functions** — proxy API que resolve o CORS
- **Deno (TypeScript)** — runtime da Edge Function
